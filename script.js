...
(function() {
  // ===== DATA - Images in same folder (no images/ path) =====
  const SERVICES_DATA = [
    ...
    { icon: "fa-graduation-cap", imgSrc: "knec.png", name: "KNEC Certificates", desc: "Guidance for KCSE/KCPE e-certificate and retrieval services." },
    { icon: "fa-user-graduate", imgSrc: "knec.png", name: "KNEC Account Registration", desc: "Help setting up and accessing your KNEC account." },
    { icon: "fa-file-invoice", imgSrc: "kra.png", name: "KRA Services", desc: "KRA PIN, returns and other online tax-related assistance." },
    { icon: "fa-hospital-user", imgSrc: "sha.png", name: "SHA Registration", desc: "Assistance with SHA registration and online services." },
    ...
    { icon: "fa-laptop", imgSrc: "ecitizen.png", name: "Online Applications", desc: "Assistance with selected online and government services." }
  ];
  ...
  const buildServices = () => {
    ...
    SERVICES_DATA.forEach((s, i) => {
      ...
      let iconHtml;
      if (s.imgSrc) {
        // Use image from same folder
        iconHtml = `<img src="${s.imgSrc}" alt="${sanitize(s.name)}" class="service-img-icon" loading="lazy" onerror="this.style.display='none';this.parentElement.innerHTML='<i class=\\'fas ${s.icon}\\' aria-hidden=\\'true\\'></i>'" />`;
      } else {
